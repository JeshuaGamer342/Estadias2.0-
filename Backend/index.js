...
try {
    ...
    if (fs.existsSync(buildPath)) {
        ...
        // Fallback para SPA
        app.get('*', (req, res) => {
            // Si la petición ya coincide con /api/* la dejamos pasar (no sobrescribir rutas API)
            if (req.path.startsWith('/api/')) return res.status(404).json({ message: 'API endpoint no encontrado' });
            return res.sendFile(path.join(buildPath, 'index.html'));
        });
    } else {
        console.log('No se encontró frontend/build. Asumiendo modo desarrollo (frontend en dev server separado).');
    }
} catch (err) {
